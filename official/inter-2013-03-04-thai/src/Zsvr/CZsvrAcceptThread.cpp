	...
	while( 1 )
	{
		if( WaitForSingleObject(m_MessageQueue.N3MessageQueue::GetEventObj(), 0) == WAIT_OBJECT_0 )
		{
			...
			if( std::tr1::get<0>(ret) == true )
			{
				...
				switch( Message )
				{
				case N3MSGID(CZsvrAcceptThread,MSG_REQ_THREAD_QUIT):
				{
					return 0;
				}
				break;
		...
		for( SESSION_CONTAINER::iterator iter = m_SessionMap.begin(); iter != m_SessionMap.end(); )
		{
			...
			if( session->m_RequestDisconnectCnt > 0 || session->m_bAuth )
			{
				if( session->m_bAuth )
				{
					...
					N3MessageMgr::GetObj()->N3MessageMgr::Post(ZsvrMgrID, N3MSGID(CZsvrMgr,MSG_ZSVR_ACCEPT), session->m_Socket, DataKey);
				}
				else
				{
...
{
	m_MessageQueue.N3MessageQueue::Put(N3MSGID(CZsvrAcceptThread,MSG_REQ_THREAD_QUIT), 0, 0);
}
