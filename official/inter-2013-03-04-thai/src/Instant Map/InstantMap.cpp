  ...
  m_ProcessFuncPtr(&CInstantMap::Process_PROCSTEP_TARGETZSVR_CREATE)
{
	...
	if( m_MapID > 0 )
	{
		N3MessageMgr::GetObj()->N3MessageMgr::Post(this->N3Object::GetMyID(), N3MSGID(CInstantMap,MSG_TARGETZSVR_CREATE_REQUEST2), 0, 0);
		m_bOK = true;
	}
}
...
void CInstantMap::Process_PROCSTEP_TARGETZSVR_CREATE(const int in_Message, const int in_wParam, const int in_lParam)
{
	switch( in_Message )
	{
	case N3MSGID(CInstantMap,MSG_NONE2):
	{
		Sleep(0);
	}
	break;
	case N3MSGID(CInstantMap,MSG_TARGETZSVR_CREATE_REQUEST2):
	{
	...
	break;
	case N3MSGID(CInstantMap,MSG_TARGETZSVR_CREATE_RESPONSE2):
	{
		...
		if( m_MapID == response->m_mapID && m_Name == response->m_mapName )
		{
			...
			if( response->m_bSuccess == true )
			{
				...
				N3MessageMgr::GetObj()->N3MessageMgr::Post(m_RequestCreateInter, N3MSGID(CInstantMap,MSG_CREATE_SUCCESS2), this->N3Object::GetMyID(), 0);
			...
			else
			{
				...
				m_State = STATE_ERROR;
				N3MessageMgr::GetObj()->N3MessageMgr::Post(m_RequestCreateInter, N3MSGID(CInstantMap,MSG_CREATE_ERROR2), 0, 0);
			}
		}
	...
	break;
	case N3MSGID(CInstantMap,MSG_DYING_WISH_REQUEST2):
	{
...
void CInstantMap::Process_PROCSTEP_OPERATION(const int in_Message, const int in_wParam, const int in_lParam)
{
	...
	break;
	case N3MSGID(CInstantMap,MSG_ERROR_FOUND_FROMZSVR2):
	{
		m_State = STATE_ERROR;
		N3MessageMgr::GetObj()->N3MessageMgr::Post(m_RequestCreateInter, N3MSGID(CInstantMap,MSG_ERROR_FOUND2), 0, 0);
	}
	break;
	case N3MSGID(CInstantMap,MSG_DYING_WISH_REQUEST2):
	{
