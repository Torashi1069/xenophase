...
lcDataTransferKeyGenerator InstancelcDataTransferKeyGenerator;
}; // namespace
...
unsigned int N3DataTransferMgr::Put(std::string in_Data)
{
	return this->N3DataTransferMgr::Put(std::vector<unsigned char>(in_Data.begin(), in_Data.end()));
}
...
unsigned int N3DataTransferMgr::Put(std::vector<unsigned char>& in_Data)
{
	...
	m_DataMap[myKey] = lcData(in_Data, Time);
