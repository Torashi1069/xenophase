...
bool CRagConnection::SendPacket(int size, const char* buf)
{
	...
	*(WORD*)buf ^= CReassemblyPacket::GetObj()->CReassemblyPacket::GetVal();
...
bool CRagConnection::RecvPacket(char* buf, WORD* len)
{
	WORD packetHeader;
...
int CRagConnection::GetPacketSize(int packetType)
{
	...
	return max(2, g_packetLenMapEx.CPacketLengthExMgr::IsFixedPacket(packetType).second);
}
...
{
	return ( g_packetLenMapEx.CPacketLengthExMgr::IsFixedPacket(packetType).first == CPacketLengthExMgr::FIXED );
}


void CRagConnection::DisableSend(bool bDisable) /// @custom
{
	m_bDisableSend = bDisable;
}
