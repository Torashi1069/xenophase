...
long __stdcall CMyMiniDumpUnhandledExceptionHandler::UnhandledExceptionHandler(EXCEPTION_POINTERS* pExceptionInfo)
{
	...
	if( hDbgHelpDll != NULL )
	{
		...
		if( pfnDump != (FARPROC)NULL )
		{
			...
			if( v4 != INVALID_HANDLE_VALUE )
			{
				...
				if( ((BOOL (WINAPI *)(HANDLE, DWORD, HANDLE, MINIDUMP_TYPE, CONST PMINIDUMP_EXCEPTION_INFORMATION, CONST PMINIDUMP_USER_STREAM_INFORMATION, CONST PMINIDUMP_CALLBACK_INFORMATION))pfnDump)(GetCurrentProcess(), GetCurrentProcessId(), v4, MiniDumpNormal, &ExInfo, NULL, NULL) )
					retval = 1;
	...
	if( !SymInitialize(GetCurrentProcess(), NULL, TRUE) ) //FIXME: 'TRUE' can make the call fail
		return retval;
	...
	while( v2 < 512 && StackFrame.AddrPC.Offset != 0 && StackWalk(IMAGE_FILE_MACHINE_I386, GetCurrentProcess(), GetCurrentThread(), &StackFrame, v11, (PREAD_PROCESS_MEMORY_ROUTINE)NULL, SymFunctionTableAccess, SymGetModuleBase, (PTRANSLATE_ADDRESS_ROUTINE)NULL) )
	{
		...
		pSymbolInfo->MaxNameLength = 253; //FIXME: wrong size?
	...
	v8.CMyFile::Write("\r\n");
	v8.CMyFile::Write("stack %08x - %08x\r\n", v11->Esp, v11->Esp + 1024); //FIXME: it's just 320 - 1, not 1024
