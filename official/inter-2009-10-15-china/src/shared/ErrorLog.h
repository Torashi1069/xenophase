...
class CErrorLog
{
	struct vtable_t
	{
		 void* (CErrorLog::* scalar_deleting_destructor)(unsigned int);
	};

	/* this+0x0 */ //const CErrorLog::`vftable'
	...
	public: void CErrorLog::CriticalErrorLogFmt(const char* in_pFileName, const int in_Line, const char* in_pFmt, ...);
	public: void CErrorLog::CriticalErrorLog(const char* buf, const int line, const char* filename);
