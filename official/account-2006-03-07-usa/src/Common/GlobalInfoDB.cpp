...
/// singleton instance
static CGlobalInfoDB g_GlobalInfo;
CGlobalInfoDB* CSingleton<CGlobalInfoDB>::ms_Singleton;
...
void CGlobalInfoDB::InitServerInfoList(bool bLogInfo)
{
	...
	while( SQL_SUCCEEDED(stmt.CDatabaseStatement::Fetch()) )
	{
		serverInfo.m_dwIP = CSingleton<CNetwork>::getSingletonPtr()->CNetwork::GetDWORDIP(serverInfo.m_szIP);
		CSingleton<CNetwork>::getSingletonPtr()->CNetwork::GetTextIP(serverInfo.m_dwIP, serverInfo.m_szIP);
		if( m_bIsColumnPrivateAdded )
		{
			serverInfo.m_dwPrivateIP = CSingleton<CNetwork>::getSingletonPtr()->CNetwork::GetDWORDIP(serverInfo.m_szPrivateIP);
			CSingleton<CNetwork>::getSingletonPtr()->CNetwork::GetTextIP(serverInfo.m_dwPrivateIP, serverInfo.m_szPrivateIP);
		}
