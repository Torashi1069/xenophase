...
{
	extern void SetMapName(char* dest, const char* src); // NpcServer/NpcEvent.cpp
...
//	mobinfo.npcType = 0;
	...
	for( int i = 0; i < num; ++i )
	{
		...
		CNPC* cpNPC = this->CScriptEventHandler::AddNpc(npcId, mapName, &mobInfo);
		if( cpNPC == NULL )
			continue; // error

		cpNPC->CNPC::SetUseTomb(bUseMobTomb);
		if( isBoss == true )
			CCharacterMgr::GetObj()->CCharacterMgr::RegisterBossInMap(cpNPC);
...
CNPC* CScriptEventHandler::AddNpc(int id, const char* mapName, NpcSvrNpcInfo* npcInfo) // 149-189
{
	PACKET_NZ_NEWNPC packet;
	packet.NAID = id;
	packet.xPos = (short)npcInfo->x + (short)npcInfo->dx;
	packet.yPos = (short)npcInfo->y + (short)npcInfo->dy;
	packet.xSize = (unsigned char)npcInfo->w;
	packet.ySize = (unsigned char)npcInfo->h;
	packet.spriteType = (short)npcInfo->sprite;
	packet.direction = (char)npcInfo->direction;
	packet.npcType = (unsigned char)npcInfo->type;
	packet.RegenMinTime = npcInfo->regenMin;
	packet.RegenMaxTime = npcInfo->regenMax;
	packet.force = false;
	packet.RegenType = npcInfo->regenType;
	memcpy_s(packet.mapName, sizeof(packet.mapName), mapName, 16);
	memcpy_s(packet.NPCName, sizeof(packet.NPCName), npcInfo->name, 24);
	packet.NPCName[countof(packet.NPCName)-1] = '\0';

	CNPC* npc = CCharacterMgr::GetObj()->CCharacterMgr::AddNPC(&packet, 0, 0);
	this->EventLog("NPC[%d] : create type[%d] zone[%s/%d] name[%s/%d] %d %d %d %d spr %d dir %d spd %d size[%d/%d/%d] regenMin:%d regenMax:%d regenType:%d\n",
		packet.NAID, packet.npcType, packet.mapName, sizeof(packet.mapName), packet.NPCName, sizeof(packet.NPCName), packet.xPos, packet.yPos, packet.xSize, packet.ySize, packet.spriteType, packet.direction, packet.force, sizeof(packet), sizeof(packet.mapName), sizeof(packet.NPCName), packet.RegenMinTime, packet.RegenMaxTime, packet.RegenType);
