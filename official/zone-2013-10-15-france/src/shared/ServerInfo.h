...
struct _SERVERINFO
{
	...
	_SERVERINFO() : bReady(false), bOverpopulation(false) { memset(this, 0, sizeof(*this)); } //FIXME: clobbers string members
};
...
public:
	...
	char* GetCharacterServerInfo();
	BOOL IsValidServer(unsigned long IP, int Type);
	...
	int GetServerID(unsigned long ip, int type);
	int GetServerIDPrivate(unsigned long ip, int type);
	...
	const unsigned long GetLocalIP();
...
private:
	...
	/* this+ 42 */ _SERVERINFO m_localServerInfo;
	/* this+152 */ std::list<int> m_MyServerSet;
	/* this+164 */ char m_characterServerInfo[1024]; // SERVER_ADDR[]
