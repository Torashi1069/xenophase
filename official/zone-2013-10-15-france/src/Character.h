...
#include "SkillTypeInfoMgr.h" // SKILLPATT_ATTACKSPELL
...
{
	TARGET_NORMAL             = 0x00,
	TARGET_EXCEPT_BOSS        = 0x01,
	TARGET_TRAP               = 0x02,
	TARGET_MERCILESS          = 0x04,
	TARGET_EXCEPT_BETWEEN_MOB = 0x08,
};
...
struct COMMAND_QUEUE
{
	...
	{
		if( skillPattern == SKILLPATT_ATTACKSPELL )
			this->par3 |= ATK_SPELL;
	}

	/* this+ 0 */ unsigned long commandID; // enum inputList
...
public:
	int GetInstantVar(const CInstantVar::enumVAR in_Var);
	void SetInstantVar(const CInstantVar::enumVAR in_Var, const int in_Data);
	int SetInstantVarIncrement(const CInstantVar::enumVAR in_Var); //TODO
	int SetInstantVarDecrease(const CInstantVar::enumVAR in_Var); //TODO
...
public:
	...
	virtual BOOL IsTarget(CCharacter* ch, unsigned long flag) = 0; // enumTARGETTYPE
