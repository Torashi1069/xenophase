...
{
public:
	bool InstantMapAdd(const int in_MapID, const CMapInfo& in_MapInfo);
	void InstantMapRemove(const int in_MapID, const std::string in_mapName);
...
public:
	...
	BOOL AddCharacter(CCharacter* pCharacter);
	void RemoveCharacter(CCharacter* pCharacter);
	void Free(CCharacter* pc);
//	class CItem * AllocItem();
	CItem* CreateItem(CITP* itp, ITEM_INFO* itemInfo);
...
//	class CSkill * GetSkillWithSKGroupUniqueID(const int, const int, const unsigned int, class CMapRes *);
	CSkill* GetSkill(int xPos, int yPos, CMapRes* mapRes);
	CSkill* GetSkill(int skillID, int xPos, int yPos, CMapRes* mapRes);
	BOOL GetSkillList(int xPos, int yPos, std::vector<CSkill*>& skList, CCharacter* exceptChar, CMapRes* mapRes);
//	bool IntersectionSkill(class CMapRes *, const int, const int, const int, const int);
//	void DestroyAllNPC();
	CNPC* AddNPC(void* p, const unsigned long parentAID, const unsigned long parentGID);
	int GetMonsterCount();
	...
	void SendToNPCServer(int len, const char* buf);
	void RegisterBossInMap(CNPC* npc);
	CNPC* FindBossInMap(int mapResID);
//	bool CheckChashTraderNpc();
//	unsigned char GetNpcType(unsigned long);
//	void ClearBossInMap();
	CPC* AllocPC();
	CPC* FindPCByGID(unsigned long GID);
	int FindPC(CCharacter* pCharacter);
	int GetPCCountIntheMap(char* mapName);
	int GetPCFreeSize();
	CPC* FindCharacterByIndex(const unsigned long in_AID, const int in_Index) const;
	...
	void PutAID(unsigned long AID);
	CCharacter* FastFindCharacter(unsigned long aID);
	...
	void SendCommand(unsigned long AID, CITZMsg* reqMsg);
	char* GetCharName(unsigned long AID);
	void GetVar(unsigned long AID, CMsg* reqMsg, CMsg* ansMsg);
	void GetVarFromPC(unsigned long AID, CMsg* reqMsg, CMsg* ansMsg);
	void ProcessAllCharacter();
	void Multicast(CMapRes* mapRes, int xPos, int yPos, int len, char* buf, int range);
	int GetCurPlayChar();
...
//	void BroadCast(class CBroadCastFunc &);
	void BroadCastExceptME(int len, char* buf, const char* ID);
...
//	class IVItEquipItem * GetEquipItem(unsigned long, unsigned long);
	char* GetGuildName(unsigned long AID);
//	void AddWarpNpcList(class CMapRes *, class CNPC *);
	void RemoveWarpNpcList(CMapRes* mapRes, CNPC* npc);
	std::vector<CNPC*> GetWarpNpcList(CMapRes* mapRes) const;
	void Flush(int mapID, int guildID, int target);
//	void ExpelOtherGuild(const int, const int);
	int GetSkillSize() const;
	int GetSkillObjCnt() const;
	int GetSkillObjCapacity() const;
	int GetItemObjCnt() const;
	int GetItemObjCapacity() const;
//	void InitLotto();
...
private:
	CNPC* AllocNPC(unsigned char type);
	CNPC* FindNPCByAID(unsigned long AID);
	CItem* SearchItem(unsigned long ITAID);
	CSkill* SearchSkill(unsigned long AID);
	void AddPCToIndexInfo(unsigned long AID, int index, unsigned long connectionTime);
	CCharacter* SearchPCFromIndexInfo(unsigned long AID);
	void DeletePCFromIndexInfo(unsigned long AID);
	void OnDisconnect(CPC* pc);
	void OnDisconnect_STEP_NONE(CPC* pc);
	void OnDisconnect_STEP_REMOVE(CPC* pc);
	void OnDisconnect_STEP_ARRANGE(CPC* pc);
	void OnDisconnect_STEP_ASYNCDB_SAVE_REQUEST(CPC* const in_pPlayer);
	void OnDisconnect_STEP_COMPLETE(CPC* pc);
	void OnDisconnect_STEP_ASYNCDB_SAVE_COMPLETE(CPC* pc);
	BOOL IsSendable(CPC* pc);
...
private:
	...
	/* this+ 28 */ CMemoryMgr<CPC> m_pcMemoryPool;
	/* this+ 64 */ CMemoryMgr<CItem> m_itemMemoryPool;
	/* this+100 */ CMemoryMgr<CSkill> m_skillMemoryPool;
	...
	friend class CPCPacketHandler; // AddPCToIndexInfo(), SearchPCFromIndexInfo()
};
