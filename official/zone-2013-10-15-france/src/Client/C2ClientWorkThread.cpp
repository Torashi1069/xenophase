...
void C2ClientWorkThread::OnMSG_REQUEST_CONNECT(unsigned int in_hClient, N3IPAddr in_ServerIPaddr, int in_ServerPortNum) // 72-108
{
	...
	if( s == INVALID_SOCKET )
	{
		...
		N3MessageMgr::GetObj()->N3MessageMgr::Post(C2ClientMgr::GetObj()->N3Object::GetMyID(), N3MSGID(C2ClientMgr,MSG_CONNECT_FAIL), in_hClient, dataKey);
		return;
	}
	...
	N3MessageMgr::GetObj()->N3MessageMgr::Post(C2ClientMgr::GetObj()->N3Object::GetMyID(), N3MSGID(C2ClientMgr,MSG_CONNECT_SUCCESS), in_hClient, s);
}
	...
	while( 1 )
	{
		if( WaitForSingleObject(m_MessageQueue.N3MessageQueue::GetEventObj(), INFINITE) == WAIT_OBJECT_0 )
		{
			...
			if( std::tr1::get<0>(mRet) == true )
			{
				...
				switch( Message )
				{
				case N3MSGID(C2ClientWorkThread,MSG_THREAD_QUIT):
				{
					return 0;
				}
				break;
				case N3MSGID(C2ClientWorkThread,MSG_REQUEST_CONNECT):
				{
...
{
	m_MessageQueue.N3MessageQueue::Put(N3MSGID(C2ClientWorkThread,MSG_THREAD_QUIT), 0, 0);
}
