...
hook_func<unsigned int (CInstantMapWorkerThread::*)()> CInstantMapWorkerThread__ThreadFunction(EXEPATH(), "CInstantMapWorkerThread::ThreadFunction");
unsigned int CInstantMapWorkerThread::ThreadFunction()
{
	return (this->*CInstantMapWorkerThread__ThreadFunction)();
	//TODO
/*
	while( 1 )
	{
		if( WaitForSingleObject(m_MessageQueue.N3MessageQueue::GetEventObj(), INFINITE) == WAIT_OBJECT_0 )
		{
			...
			if( std::tr1::get<0>(mRet) == true )
			{
				...
				switch( Message )
				{
				case N3MSGID(CInstantMapWorkerThread,MSG_REQ_THREAD_QUIT2):
				{
					return 0;
				}
				break;
				case N3MSGID(CInstantMapWorkerThread,MSG_REQUEST_WORK2):
				{
					CIMWTWork* work = reinterpret_cast<CIMWTWork*>(std::tr1::get<2>(mRet));
					unsigned int senderId = std::tr1::get<3>(mRet);

					work->Run();
					N3MessageMgr::GetObj()->N3MessageMgr::Post(senderId, N3MSGID(MSG_RESPONSE_WORK2,MSG_REQUEST_WORK2), work, 0);
				}
				break;
				default:
				{
...
{
	m_MessageQueue.N3MessageQueue::Put(N3MSGID(CInstantMapWorkerThread,MSG_REQ_THREAD_QUIT2), 0, 0);
}
