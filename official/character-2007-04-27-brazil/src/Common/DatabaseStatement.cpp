...
bool CDatabaseStatement::BindFloatColumn(int nColumn, void* pData)
{
	...
	if( !SQL_SUCCEEDED(SQLBindCol(m_hStmt, nColumn, SQL_C_FLOAT, pData, 0, &cbNum)) )
	{
		char szSQLError[MAX_BUFFER_LENGTH] = {};
		GetSQLErrorString(m_hEnv, m_hDbc, m_hStmt, szSQLError);
		CSingleton<CReporter>::getSingletonPtr()->CReporter::Log("CDatabaseStatement::BindFloatColumn() - SQLBindCol() fail(%s - %s)", m_pszDSN, szSQLError);
		return false;
	}
...
struct SODBCPARAM
{
	...
	/* this+ 8 */ __int64 m_nParam;
	/* this+16 */ char m_szParam[256];
};
...
BOOL CDatabaseStatement::Query(char* lpszQuery, ...)
{
	...
	{// simple query (no parameters)
		...
		if( !SQL_SUCCEEDED(sqlResult) )
		{
			char szSQLError[MAX_BUFFER_LENGTH] = {};
			GetSQLErrorString(m_hEnv, m_hDbc, m_hStmt, szSQLError);
			CSingleton<CReporter>::getSingletonPtr()->CReporter::FatalError("CDatabaseStatement::Query() - SQLExecDirect() fail(%s - %s)", m_pszDSN, szSQLError);
			return FALSE;
		}
	...
	for( int i = 0; i < nParam && i < MAX_ODBC_PARAM; ++i )
	{
		...
		{// bigint (<value>)
			...
			odbcParam[i].m_nParam = value;
			sqlResult = SQLBindParameter(m_hStmt, i + 1, SQL_PARAM_INPUT, SQL_C_SBIGINT, SQL_BIGINT, 0, 0, &odbcParam[i].m_nParam, 0, NULL);
		}
		break;
