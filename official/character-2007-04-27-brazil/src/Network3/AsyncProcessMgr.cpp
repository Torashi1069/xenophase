...
bool CAsyncProcessMgr::Create(int nThreadNum, int nPacketPoolCount)
{
	...
	{
		CSingleton<CReporter>::getSingletonPtr()->CReporter::Log("%s - m_async_packet_pool creation failed\n", __FUNCTION__);
		return false;
	}
	...
	for( int i = 0; i < nThreadNum; ++i )
	{
		...
		{
			if( errno == EAGAIN )
				CSingleton<CReporter>::getSingletonPtr()->CReporter::Log("CreateMemberThread< CAsyncProcessMgr > ProcessThread fail! errno EAGAIN(%d)", errno);
			else if( errno == EINVAL )
				CSingleton<CReporter>::getSingletonPtr()->CReporter::Log("CreateMemberThread< CAsyncProcessMgr > ProcessThread fail! errno EINVAL(%d)", errno);
			else
				CSingleton<CReporter>::getSingletonPtr()->CReporter::Log("CreateMemberThread< CAsyncProcessMgr > ProcessThread fail! errno %d", errno);
...
void CAsyncProcessMgr::ProcessThread(void* lpParam)
{
	...
	CSingleton<CReporter>::getSingletonPtr()->CReporter::Log("%s out\n", __FUNCTION__);
}
