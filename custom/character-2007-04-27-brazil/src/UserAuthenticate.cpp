...
void CUserAuthenticate::Insert(DWORD dwAID, __SUSERAUTHENTICATE__& object)
{
	...
	if( m_pAuthenticate == NULL )
	{
		CSingleton<CReporter>::getSingletonPtr()->CReporter::Log("m_pAuthenticate is 0 func %s\n", __FUNCTION__);
		return;
	}
	...
	if( it == auth->m_Map.end() )
	{
		...
		if( pObj == NULL )
		{
			CSingleton<CReporter>::getSingletonPtr()->CReporter::Log("%s error !!!!! new fail.. aid %u \n", __FUNCTION__, dwAID);
			auth->m_csMap.Leave();
			return;
		}
