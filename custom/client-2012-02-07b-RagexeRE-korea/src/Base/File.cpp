...
bool CFile::Open(const char* fName, unsigned int nOpenFlags, bool bMakeDataPath, bool bReadFolderOnly)
{
	if( nOpenFlags == 0 )
	{
		if( bMakeDataPath )
		...
		m_cursor = 0;
		m_buf = (char*)g_fileMgr.CFileMgr::GetData(m_fileName, &m_size, bReadFolderOnly);
		if( m_buf == NULL )
			return false;
...
bool CFile::IsFileExist(const char* fName, bool bReadFolderOnly)
{
	char fullName[256];
	CFile::MakeFileName(fullName, fName);

	return g_fileMgr.CFileMgr::IsDataExist(fullName, bReadFolderOnly);
}
