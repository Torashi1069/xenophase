...
void XMLElement::AddChild(XMLElement* element)
{
	if( m_child == NULL )
		...
		m_child->XMLElement::AddNext(element);
}
...
XMLElement* XMLElement::FindChild(const char* name)
{
	if( m_child == NULL || strcmp(m_child->m_name, name) == 0 )
		...
		return m_child->XMLElement::FindNext(name);
}
...
const char* XMLDocument::ReadContents(XMLElement* element, const char* document, const char* end)
{
	...
	while( p != end )
	{
		...
		{
			if( strcmp(element->XMLElement::GetName(), "registrationweb") == 0 )
			{// raw ampersands in URL
				++p;
				continue;
			}
...
bool XMLDocument::IsTag(const char* document)
{
	return ( *document == '<' );
}


bool XMLDocument::IsEntity(const char* document)
{
	return ( *document == '&' );
}


const char* XMLDocument::SkipEntity(const char* document, const char* end)
{
	const char* i;
