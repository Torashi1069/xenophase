...
int __cdecl main(int argc, const char** argv, const char** envp)
{
	...
	const char* PASSWORD_LOG = g_scrptConfig.CInfoScript::GetInfo("PASSWORD_LOG");
	CSingleton<CConnectLogDB>::getSingletonPtr()->CDatabase::Connect("loger", PASSWORD_LOG, "Log");

	for( int i = 0; i < countof(g_AccountDB); ++i )
	{
		const char* PASSWORD_NLOGIN = g_scrptConfig.CInfoScript::GetInfo("PASSWORD_NLOGIN");
	...
	const char* PASSWORD_GLOBALINFO = g_scrptConfig.CInfoScript::GetInfo("PASSWORD_GLOBALINFO");
	...
	// [patch] bind IP address sanity check
	SSERVERINFO LocalServerInfo;
	CSingleton<CServerInfoList>::getSingletonPtr()->CServerInfoList::GetServerInfo(&LocalServerInfo, SID);
	if( !CSingleton<CGlobalInfoDB>::getSingletonPtr()->CGlobalInfoDB::IsColumnPrivateAdded() )
		CSingleton<CReporter>::getSingletonPtr()->CReporter::FatalError("This server requires PrivateIP and PrivatePort columns in ServerInfo table");
	if( LocalServerInfo.m_dwIP != CSingleton<CNetwork>::getSingletonPtr()->CNetwork::GetLocalIP(true) )
		CSingleton<CReporter>::getSingletonPtr()->CReporter::FatalError("PUBLIC_IP misconfigured, check account.config.txt and ServerInfo table");
	if( LocalServerInfo.m_dwPrivateIP != CSingleton<CNetwork>::getSingletonPtr()->CNetwork::GetLocalIP(false) )
		CSingleton<CReporter>::getSingletonPtr()->CReporter::FatalError("PRIVATE_IP misconfigured, check account.config.txt and ServerInfo table");
	...
	#define CONF(X) ( g_scrptConfig.CInfoScript::GetInfo(X) != NULL ) ? atoi(g_scrptConfig.CInfoScript::GetInfo(X))
	/////////////////////////////////////////////////////////////////////////////
	const bool WUCS_ENABLE = CONF("WUCS_ENABLE") == 1 : false;
	if( WUCS_ENABLE ) // [patch] WUCS disable option
	{
	...
	const int WUCS_NETWORK_BUFSIZE = CONF("WUCS_NETWORK_BUFSIZE") : 3000000;
	const int WUCS_PROCESS_OBJ_NUM = CONF("WUCS_PROCESS_OBJ_NUM") : 1;
	const int WUCS_PROCESS_THREADS = CONF("WUCS_PROCESS_THREADS") : 1;
	const int WUCS_WORKER_THREADS = CONF("WUCS_WORKER_THREADS") : 1;
	const int WUCS_SLEEP_RATIO = 1; // [patch] changed 3 to 1 to reduce cpu usage
	...
	/////////////////////////////////////////////////////////////////////////////
	const DWORD CHAR_IP = LocalServerInfo.m_dwPrivateIP;
	const WORD CHAR_PORT = (WORD)LocalServerInfo.m_nPrivatePort;
	const int CHAR_NETWORK_BUFSIZE = CONF("CHAR_NETWORK_BUFSIZE") : 3000000;
	int CHAR_PROCESS_OBJ_NUM = CONF("CHAR_PROCESS_OBJ_NUM") : 6;
	int CHAR_PROCESS_THREADS = CONF("CHAR_PROCESS_THREADS") : 5;
	int CHAR_WORKER_THREADS = CONF("CHAR_WORKER_THREADS") : 3;
	int CHAR_SLEEP_RATIO = 1; // [patch] changed 3 to 1 to reduce cpu usage
	...
	Sleep(1000);
	/////////////////////////////////////////////////////////////////////////////
	const DWORD USER_IP = LocalServerInfo.m_dwIP;
	const WORD USER_PORT = (WORD)LocalServerInfo.m_nPort;
	const int USER_NETWORK_BUFSIZE = CONF("USER_NETWORK_BUFSIZE") : 10000;
	const int USER_PROCESS_OBJ_NUM = CONF("USER_PROCESS_OBJ_NUM") : 5000;
	const int USER_PROCESS_THREADS = CONF("USER_PROCESS_THREADS") : 10;
	const int USER_WORKER_THREADS  = CONF("USER_WORKER_THREADS") : 3;
...
void __cdecl UpdateInfoThread(void* lpVoid)
{
	...
	while( 1 )
	{
		...
		sprintf(szInfo, "(%d,%d)(C%d,%d)(%d/%d)(H%d)(%d/%d)(B%d,%d) %04d:%02d:%02d %s v(%s %s)", // [patch] build date in window title
