...
bool CCharSvrProcess::OnConnect()
{
	...
	if( CSingleton<CServerInfoList>::getSingletonPtr()->CServerInfoList::GetSID(m_dwIP, SERVERTYPE_CHARACTER) == 0 ) // [patch] more flexible remote IP/type lookup
	{
...
bool CCharSvrProcess::ProcessPacket_CHAR_EXIT(char* pPacket)
{
	...
	if( g_userList.CUserList::Bring(AID, &user) == TRUE )
	{
		...
		CSingleton<CConnectLog>::getSingletonPtr()->CConnectLog::AddConnectLog(user.m_szID, user.m_nIP, AID, TRUE); // [patch] login/logout event logging
	}
