...
void CZSPacketHandler::OnBroadCast(int len)
{
	...
	{
		/// [patch] custom broadcast log format with daily log rotation
		char szFileName[256];
		time_t t = time(NULL);
		strftime(szFileName, sizeof(szFileName), "BroadCastLog(%Y%m%d).log", localtime(&t));
		...
		if( fp != NULL )
		{
			SYSTEMTIME st;
			GetLocalTime(&st);
			fprintf(fp, "[%02d-%02d-%02d %02d:%02d:%02d.%03d] %s\n", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds, pZI_BROADCAST->Text);
			fclose(fp);
		}
		/// [/patch]
	}
...
void CZSPacketHandler::OnZI_INSTANTMAP_CREATE_RES(const PACKET_ZI_INSTANTMAP_CREATE_RES& in_packet)
{
	...
	N3MessageMgr::GetObj()->N3MessageMgr::Post(in_packet.hRequestN2Obj, N3MSG(CInstantMap,MSG_TARGETZSVR_CREATE_RESPONSE2), reinterpret_cast<unsigned int>(response), 0);
}


void CZSPacketHandler::OnZI_INSTANTMAP_ERROR(const PACKET_ZI_INSTANTMAP_ERROR& in_packet)
{
	N3MessageMgr::GetObj()->N3MessageMgr::Post(in_packet.hRequestN2Obj, N3MSG(CInstantMap,MSG_ERROR_FOUND_FROMZSVR2), in_packet.mapID, in_packet.why);
