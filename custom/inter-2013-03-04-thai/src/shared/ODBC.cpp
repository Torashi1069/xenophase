...
BOOL CODBC::Init(const char* FileDSN, const char* PASS)
{
	...
	std::string strConnectInfo = std::string("FILEDSN=.\\") + std::string(FileDSN) + std::string(";PWD=") + std::string(PASS);
	char ConnStrOut[255] = {};
	...
	if( !SQL_SUCCEEDED(m_retcode) )
	{
		...
		char Text[1024];
		sprintf_s(Text, countof(Text), "DBServer connect error DSNFILE :%s", FileDSN);
		NCriticalErrorLog::GetObj()->NFileLog::WriteFmt(__FILE__, __LINE__, "%s", Text);
