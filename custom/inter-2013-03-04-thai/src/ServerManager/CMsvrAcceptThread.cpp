	...
	while( 1 )
	{
		if( WaitForSingleObject(m_MessageQueue.N3MessageQueue::GetEventObj(), 0) == WAIT_OBJECT_0 )
		{
			...
			if( std::tr1::get<0>(ret) == true )
			{
				...
				switch( Message )
				{
				case N3MSG(CMsvrAcceptThread,MSG_REQ_THREAD_QUIT):
					return 0;
				break;
				default:
		...
		N3MessageMgr::GetObj()->N3MessageMgr::Post(CMsvrMgr::GetObj()->N3Object::GetMyID(), N3MSG(CMsvrMgr,MSG_MSVR_ACCEPT), s, remoteAddr.sin_addr.s_addr);
	}
}
...
{
	m_MessageQueue.N3MessageQueue::Put(N3MSG(CMsvrAcceptThread,MSG_REQ_THREAD_QUIT), 0, 0);
}
