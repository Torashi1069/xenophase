...
HOOKED bool CScriptCompiler::InitDungenNpcScript(void) // line 309
{
//	return (this->*_InitDungenNpcScript)();
	...
	if( !scriptList.CLoadDGScriptList::Load(GetScriptFileName(DUNGEON_NPC_LIST)) || !scriptList.CLoadDGScriptList::Parsing() )
		return false;
	...
	while( scriptList.GetListCount() > 0 )
	{
		...
		while( scList.size() != 0 )
		{
			...
			scList.pop_front();
			g_pathDataNpc->SetFileName(scFile.c_str());
			this->CScriptCompiler::Run(g_pathDataNpc, 0);
		}
