...
HOOKED int CPC::isNotifyPosition(short xPos, short yPos)
{
//	return (this->*_isNotifyPosition)(xPos, yPos);

	if( abs(xPos - m_characterInfo.NotifyPositionX) <= 2
	&&  abs(yPos - m_characterInfo.NotifyPositionY) <= 2 )
	{
		return 0;
	}
	else
...
HOOKED short CPC::GetItemDef(int isServer, int isClient)
{
	isServer = 1;
...
HOOKED void CPC::Send(int len, char* buf)
{
	if( m_asyncStream->GetSocket() == INVALID_SOCKET )
	{// skip useless writes
		m_lastSendTime = timeGetTime();
		m_characterTimeInfo.lastPacketArrivedTime = m_lastSendTime;
		return;
	}
...
HOOKED void CPC::ForceSend(int len, char* buf)
{
	if( m_asyncStream->GetSocket() == INVALID_SOCKET )
	{// skip useless writes
		m_lastSendTime = timeGetTime();
...
HOOKED void CPC::CancelMCStore(void)
{
	(this->*_CancelMCStore)();
	if( this->m_asyncStream->GetSocket() == INVALID_SOCKET )
		this->m_packetHandler.OnClose();
	return;
...
HOOKED void CPC::OnMakeCouple(unsigned long gid)
{
//	return (this->*_OnMakeCouple)(gid);
	...
	v4->UpdateBodyItemCountByNPC(2634, 1, true);
	v2->UpdateBodyItemCountByNPC(2635, 1, true);
...
HOOKED void CPC::SetEffectByGospel(CCharacter* ch)
{
//	return (this->*_SetEffectByGospel)(ch);
	...
	case 45:
		if( type == PC_TYPE )
		{
			((CPC*)ch)->m_pcBattle.CPCBattle::SetATKPercentInfo(SKID_PA_GOSPEL, 60000, 32);
			((CPC*)ch)->m_pcBattle.CPCBattle::SetDEFPercentInfo(SKID_PA_GOSPEL, 60000, -55);
		}
		else
		if( type == NPC_MOB_TYPE )
		{
			((CNPC*)ch)->CNPC::SetATKPercentInfo(SKID_PA_GOSPEL, 60000, 32);
			((CNPC*)ch)->CNPC::SetDEFPercentInfo(SKID_PA_GOSPEL, 60000, -55);
		}

		g_characterMgr->CCharacterMgr::SendMsg(this, ch, CM_SETEFFECTIVE, EFST_PROVOKE, 60000, 11, 0);
...
HOOKED int CPC::IsCrazyKiller(void)
{
//	return (this->*_IsCrazyKiller)();
	return 0; // disable the killer system bonus
...
HOOKED int CPC::UpdatePKInfo(CPC* winner, CPC* loser)
{
//	return (this->*_UpdatePKInfo)(winner, loser);
	...
	// can't kill the same player twice in a row and gain points (custom)
	if( strcmp(winner->m_PKInfo.killName, loser->m_characterInfo.characterName) == 0 )
		return 1;
	...
	{
		win = 1;
		lose = 2; // lose 5 pts on official (custom)
	}
...
HOOKED void CPC::SetPKInfo(int win, int lose, char* kill_name, char* killed_name, int bLoserCrazyKiller)
{
//	return (this->*_SetPKInfo)(win, lose, kill_name, killed_name, bLoserCrazyKiller);
	...
	if( lose != 0 )
	{
		...
		unsigned __int64 expire_time_loser = CSettings::GetObj()->killer_expire_time_loser;
		unsigned __int64 t = expire_time_loser + m_PKInfo.expireTime.dwLowDateTime + ((unsigned __int64)m_PKInfo.expireTime.dwHighDateTime << 32);
		m_PKInfo.expireTime.dwLowDateTime += (DWORD)expire_time_loser;
		m_PKInfo.expireTime.dwHighDateTime = (DWORD)(t >> 32);
		m_PKInfo.expireTimeCheck = timeGetTime() + (DWORD)(expire_time_loser / 10000);
	...
	if( win != 0 )
	{
		m_PKInfo.noPointTime = timeGetTime() + (DWORD)CSettings::GetObj()->killer_expire_time_winner;

		int manner_point = CSettings::GetObj()->killer_manner_point;
		if( manner_point > 0 && bLoserCrazyKiller == 0 )
			g_characterMgr->CCharacterMgr::SendMsg(this, this, CM_NOTIFY_MANNER_POINT_GIVEN, 1, manner_point, 0, 0);
...
HOOKED void CPC::SetKillerSystem(void)
{
//	return (this->*_SetKillerSystem)();
	return; // disable packet send
...
HOOKED void CPC::NotifyPKInfo(void)
{
...
HOOKED int CPC::GetAttackFinalDamage(CCharacter* other, int position, int skillProperty, int plusATTPowerPercent, int plusATTSuccessPercent, int type, int isRangeAttack, unsigned short SKID, short skLevel)
{
	int result = (this->*_GetAttackFinalDamage2)(other, position, skillProperty, plusATTPowerPercent, plusATTSuccessPercent, type, isRangeAttack, SKID, skLevel);
	int jobid;
	if( other->GetParameterValue(VAR_JOB, jobid) && jobid == JT_EMPELIUM )
		result = ModifyDamage_Emperium(result);
	return result;
...
HOOKED void CPC::OnDead(unsigned long enemyAID, int& isRestart)
{
	if( IsSetting(SETTING_KILLER_SYSTEM) && !this->CPC::IsFirstJobORLowLevel() && this->IsItPossible(MAPPROPERTY_PKSERVER) && !this->IsItPossible(MAPPROPERTY_EXP) && enemyAID != this->GetAccountID() )
	{
		CPC* pc = (CPC*)g_characterMgr->CCharacterMgr::FindCharacter(enemyAID, PC_TYPE);
		if( pc != NULL )
			this->CPC::UpdatePKInfo(pc, this);
	}
...
/// @custom
void CPC::SetItem(int itid, int value)
{
	g_characterMgr->SendMsg(this, m_characterInfo.accountID, CM_SET_ITEM, m_characterInfo.accountID, itid, value, 0);
}


/// @custom
int CPC::GetItem(int itid)
{
	CMsg reqmsg, ansmsg;
	reqmsg.CMsg::SetMsgID(1);
	reqmsg.par1 = itid;
	g_characterMgr->GetVar(m_characterInfo.accountID, &reqmsg, &ansmsg);
	return ansmsg.par1;
}
