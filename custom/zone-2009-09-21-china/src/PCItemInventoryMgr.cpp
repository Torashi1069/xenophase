...
#include "Custom/umage.hpp" //PARTY_ITEM_SHARE_DISTANCE
#include "globals.hpp"
#include "struct.hpp"
...
HOOKED void CPCItemInventoryMgr::OnPickUpItem(unsigned long ITAID)
{
//	return (this->*_OnPickUpItem)(ITAID);
	...
	// choose the char to actually get the item.
	CPC* pc = NULL;
	if( this->m_owner->m_characterInfo.PartyItemDivisionRule && this->m_owner->m_groupMember.size() > 1 )
		pc = this->CPCItemInventoryMgr::RandomDivision(item);
	...
	if( !(mapID == this->m_owner->m_characterInfo.mapID && abs(otherXPos - myXpos) < 5 && abs(otherYPos - myYpos) < 5) )
		return; //TODO: you can loot at 5 cell range? exploitable...
	...
	if( !pc->m_pcitemInventoryMgr.Add(ivit, 0, 0, 1, 0) )
	{
		pc->m_pcitemInventoryMgr.FreeInventoryItem((InventoryItem *)ivit);
		return;
	}

	pc->m_pcitemInventoryMgr.NotifyPickUpToParty(ivit);
	g_itemLog->CItemLog::AddItemLog(1, pc, NULL, ivit, 0, ivit->m_count, NULL);
	pc->m_pcitemInventoryMgr.RefreshItemWeight();
...
HOOKED void CPCItemInventoryMgr::PurchaseItemFromMCStore(unsigned long targetAID, int contentLength, char* buf)
{
//	return (this->*_PurchaseItemFromMCStore)(targetAID, contentLength, buf);

	(this->*_PurchaseItemFromMCStore)(targetAID, contentLength, buf);
	CPC* pc = (CPC*) g_characterMgr->CCharacterMgr::FindCharacter(targetAID, PC_TYPE);
	if( pc && pc->m_pcitemInventoryMgr.m_mcStoreItem.GetSize() == 0 )
	{
		PACKET_CZ_REQ_CLOSESTORE packet;
		packet.PacketType = HEADER_CZ_REQ_CLOSESTORE;
		pc->m_asyncStream->Recv(sizeof(packet), (char*)&packet);
	}
	return;
...
CPC* CPCItemInventoryMgr::RandomDivision(CItem* item)
{
//	return (this->*_RandomDivision)(item);
	...
	// first, make a list of characters eligible for obtaining the item:
	std::vector<CPC*> eligible;
	for( mystd::list<unsigned long>::iterator it = m_owner->m_groupMember.begin(); it != m_owner->m_groupMember.end(); ++it )
	{
		CPC* v5 = (CPC*) g_characterMgr->CCharacterMgr::FindCharacter((*it), PC_TYPE);
		if( v5 == NULL )
			continue; // not online
		if( v5->IsDeadState() )
			continue; // dead
		if( !v5->IsMyArea(item, PARTY_ITEM_SHARE_DISTANCE) )
			continue; // too far away
		if( v6 + v5->m_pcitemInventoryMgr.CPCItemInventoryMgr::GetBodyItemWeight() > v5->CPC::GetBodyMaxItemWeight() )
			continue; // too heavy
		//TODO: check inventory capacity, gm privilege, etc? or just add a big while() loop?
		eligible.push_back(v5);
	}

	if( eligible.size() == 0 )
		return NULL;

	// then, choose the lucky member at random
	return eligible[GetServerRandom(0, 20000) % eligible.size()];
}
