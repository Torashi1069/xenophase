...
HOOKED int GetServerRandom(int nMin, int nMax) // line 3068
{
...
#else
#include "sfmt/sfmt.hpp"
__declspec(thread) sfmt_19937_generator* _random = NULL;
HOOKED int GetServerRandom(int nMin, int nMax)
{
	if( _random == NULL )
	{
		_random = new sfmt_19937_generator;
		_random->seed(timeGetTime() + (unsigned int)time(NULL) + _getpid() + GetCurrentProcessId());
	}

	return min(nMin, nMax) + _random->rand32() % (unsigned int)(nMax - nMin + 1);
}
#endif
