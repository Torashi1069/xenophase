...
#include "Custom/umage.hpp" // NPC_PROCESS_INTERVAL, DIFF_TIME()
...
HOOKED void CCharacterMgr::Init(int MaxCharacterCount) // line 73-95
{
	MaxCharacterCount = CSettings::GetObj()->max_character_count;
...
void CCharacterMgr::Free(CCharacter* pc) // line 218-276
{
	...
	case NPC_MERSOL_TYPE:
		pc->GetAccountID(); //nop
		if( pc->GetAccountID() <= MAX_AID_COUNT )
...
HOOKED CCharacter* CCharacterMgr::FindCharacter(unsigned long AID, unsigned long Type) // line 906-928
{
	if( Type == NPC_MOB_TYPE || Type == NPC_EVT_TYPE || Type == NPC_PET_TYPE || Type == NPC_HO_TYPE || Type == NPC_MERSOL_TYPE )
		Type = NPC_TYPE;
...
/// @patch backport "output disconnect log" setting from 20100527ZoneProcess - r2617
hook_method<void (CCharacterMgr::*)(void)> CCharacterMgr::_ProcessAllCharacter(SERVER, "CCharacterMgr::ProcessAllCharacter");
HOOKED void CCharacterMgr::ProcessAllCharacter(void) // line 1282-1412
{
	...
	if ( g_zsAcceptHandler->GetAcceptable() )
	{
		...
		for( int v1 = 0; v1 < this->m_PCCount; ++v1 )
		{
			...
			else
			{
				...
				if ( v22 != NULL )
				{
					if ( v22->m_DisconnetTime + 60000 > timeGetTime() && CSettings::GetObj()->m_bIsOutputDisconnectLog != 0 )
		...
		static int last_npc_time = timeGetTime();
		if( DIFF_TIME(timeGetTime(), last_npc_time) >= NPC_PROCESS_INTERVAL )
		{
		last_npc_time = timeGetTime();
...
/// @custom
void CCharacterMgr::GlobalChat(enum enumGlobalChannel channel, const char* message)
{
	char buf[512];
	PACKET_ZC_NPC_CHAT& packet = *(PACKET_ZC_NPC_CHAT*)buf;
	packet.PacketType = HEADER_ZC_NPC_CHAT;
	packet.packetLength = sizeof(PACKET_ZC_NPC_CHAT) + strlen(message) + 1;
	packet.accountID = 0; // dummy
	packet.color = Channels[channel].color;
	strcpy(packet.Text, message);
	...
	for( int i = 0; i < m_PCCount; ++i )
	{
		...
		if( pc->m_isMapLoading )
			continue;

		int value = globalchat_getitem(pc);

		bool joined = ( (value & (1 << channel)) != 0 );
		if( !joined )
			continue;

		pc->CPC::Send(packet.packetLength, (char*)&packet);
	}
}
