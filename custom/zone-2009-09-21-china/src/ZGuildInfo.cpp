...
#include "Custom/umage.hpp" //DIFF_TIME()
#include "globals.hpp"
...
HOOKED void CZGuildInfo::SendGuildNotice(unsigned long AID) // line 386
{
	CPC* pc = (CPC*)g_characterMgr->CCharacterMgr::FindCharacter(AID, PC_TYPE);
	if( pc != NULL && DIFF_TIME(timeGetTime(), pc->m_loadingCompleteTime) < 20 && DIFF_TIME(timeGetTime(), pc->m_characterTimeInfo.connectedTime) >= 5000 )
		return;
...
HOOKED void CZGuildInfo::OnNotifySkillData(char* Buf, unsigned short Len) // line 726
{
