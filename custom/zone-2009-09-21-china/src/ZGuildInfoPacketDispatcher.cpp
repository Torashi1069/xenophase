...
/// @patch change guild maxUserNum to 12 players (w/ GD_EXTENSION)
...
HOOKED void CZGuildInfoPacketDispatcher::GetGuildInfo(unsigned long GDID, short PacketType, unsigned short Len, char* Buf)
{
//	return (this->*_GetGuildInfo)(GDID, PacketType, Len, Buf);
	...
	if( lv >= 1 )
	{
		m_guildInfo->CGuild::SetMaxUserNum(2 * lv + 8);
		Guild.MaxUserNum = 2 * lv + 8;
