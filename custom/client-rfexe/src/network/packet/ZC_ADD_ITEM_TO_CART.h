struct PROTO_ZC_ADD_ITEM_TO_CART
{
	short PacketType;
	short index;
	int count;
...
struct PACKET_ZC_ADD_ITEM_TO_CART
{
	...
	{
		TPacket<PACKET_ZC_ADD_ITEM_TO_CART> in(p);
		TPacket<PROTO_ZC_ADD_ITEM_TO_CART> out;
	...
	{
		TPacket<PROTO_ZC_ADD_ITEM_TO_CART> in(p);
		TPacket<PACKET_ZC_ADD_ITEM_TO_CART> out;
...
struct PACKET_ZC_ADD_ITEM_TO_CART2
{
	...
	static Packet* ToProto(short PacketType, Packet* p)
	{
		TPacket<PACKET_ZC_ADD_ITEM_TO_CART2> in(p);
		TPacket<PROTO_ZC_ADD_ITEM_TO_CART> out;
	...
	{
		TPacket<PROTO_ZC_ADD_ITEM_TO_CART> in(p);
		TPacket<PACKET_ZC_ADD_ITEM_TO_CART2> out;
