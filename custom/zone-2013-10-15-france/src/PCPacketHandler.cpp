...
*/
	...
	{// 2488-2491
		...
		//TODO
		/*
		if( !g_zoneScript.CZoneScript::SearchID(m_pc->CPC::GetAccountName(), CHECK_MOVEAID) )
			break;
	...
	{// 3509-3526
		...
		//TODO
		/*
		switch( GetPureJob(m_pc->m_characterInfo.job) )
		{
		case 4059: // JT_GUILLOTINE_CROSS
			m_pc->CPC::GC_POISONINGWEAPONoperation(packet.id);
		break;
		case 4058: // JT_MECHANIC
			m_pc->CPC::NC_MAGICDECOY_operate(packet.id);
		break;
		case 4055: // JT_WARLOCK
	...
	{// 3821-3833
		...
		/*
		CPC* other = (CPC*)CCharacterMgr::GetObj()->CCharacterMgr::FindCharacter(packet.AID, PC_TYPE);
		if( other == NULL || !packet.answer || !other->m_marriageInfo.bMarried )
			break;
	...
	{// 3839-3864
		...
		if( packet.which == 10 )
		{
			if( m_pc->m_characterInfo.job < JT_NOVICE_H || m_pc->m_characterInfo.job > JT_CHICKEN2_H
			 ...
			 && m_pc->CPC::ConsumeItem(ITID_BLACK_GEMSTONE, 1, 0, 0) )
			{
	...
	{// 4804-4818
		//TODO
		/*
		g_UserBlockMgr.CUserBlockMgr::AddR2(TYPE_WARNNING, REASON_UNKNOWN_PACKET, m_pc->GetAccountID(), CGVar::GetObj()->CGVar::GetUserBlockSecond_when_found_unknown_packet());
...
void CPCPacketHandler::OnCZ_PARTY_CONFIG(const PACKET_CZ_PARTY_CONFIG& in_packet) // ????-???? (1201-1210)
{
	//TODO
	/*
	m_pc->m_characterInfo.Party.Bit.bRefuseJoinMsg = packet.bRefuseJoinMsg;
...
/*
	...
	m_pc->m_isLessEffect = packet.isLess;
*/
}
