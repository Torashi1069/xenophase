...
BOOL CServerInfo::GetSvrInfoFromDB(const int in_hExpectSvrType, const int in_hExpectSID)
{
	...
	if( ExistColumn )
		m_retcode = SQLPrepare(m_hstmt, (SQLCHAR*)"select  SID, Type, IP, Port, DestinationOneSID, DestinationTwoSID, SvrName, PrivateIP, PrivatePort from ServerInfo", SQL_NTS);
	else
		m_retcode = SQLPrepare(m_hstmt, (SQLCHAR*)"select  SID, Type, IP, Port, DestinationOneSID, DestinationTwoSID, SvrName from ServerInfo", SQL_NTS);
...
void CServerInfo::SetCharacterServerInfo()
{
	for( std::map<int,_SERVERINFO*>::iterator ServerInfoiterator = m_serverInfoMap.begin(); ServerInfoiterator != m_serverInfoMap.end(); ++ServerInfoiterator )
	{
		...
		if( info->Type == CHARACTERSERVERTYPE || info->Type == ENGLISHCHARACTERSERVERTYPE )
		{
			...
			serverAddress.port = LOWORD(info->Port);
...
int CServerInfo::GetCharacterServerInfoLength()
{
...
char* CServerInfo::GetCharacterServerInfo()
{
	return m_characterServerInfo;
}


BOOL CServerInfo::IsValidServer(unsigned long IP, int Type)
{
...
const unsigned long CServerInfo::GetLocalIP()
{
	return m_localServerInfo.ip;
}
...
int CServerInfo::GetServerID(unsigned long ip, int type)
{
	for( std::map<int,_SERVERINFO*>::iterator iterator = m_serverInfoMap.begin(); iterator != m_serverInfoMap.end(); ++iterator )
	{
		_SERVERINFO* info = iterator->second;
...
void CServerInfo::GetZoneServerIPPrivate(std::set<unsigned long>& zoneset)
{
	for( std::map<int,_SERVERINFO*>::iterator iterator = m_serverInfoMap.begin(); iterator != m_serverInfoMap.end(); ++iterator )
	{
		if( iterator->second->Type == ZONEPROCESSSERVERTYPE )
		{
			zoneset.insert(iterator->second->PrivateIP);
			zoneset.insert(iterator->second->ip);
		}
	}
}
...
BOOL CServerInfo::IsExistColumn()
{
	...
	if( st.CDBStatement::Init(m_hdbc) )
	{
		...
		if( SQL_SUCCEEDED(m_retcode) )
		{
			m_retcode = SQLPrepare(m_hstmt, (SQLCHAR*)"EXEC sp_columns @table_name = ServerInfo, @column_name  = PrivateIP", SQL_NTS);
