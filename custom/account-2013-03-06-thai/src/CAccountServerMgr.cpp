...
std::pair<bool,std::string> CAccountServerMgr::Startup1st()
{
	...
	// [patch] set timer resolution
	MMRESULT ret = timeBeginPeriod(1);
	if( ret != TIMERR_NOERROR )
		CEnterTraceLog::GetObj()->NFileLog::WriteFmt(__FILE__, __LINE__, "timeBeginPeriod failed, ret %u\n", ret);

	// [patch] initialize network
	if( !CNetwork::GetObj()->CNetwork::InitializeNetwork() )
		CEnterTraceLog::GetObj()->CEnterTraceLog::FatalError(__FILE__, __LINE__, "gNetwork.InitializeNetwork failed\n");
	...
	// [patch] login/logout event logging
	if( !CSingleton<CConnectLogDB>::getSingletonPtr()->CDatabase::Connect("connectlog.dsn", dbpw) )
		CEnterTraceLog::GetObj()->CEnterTraceLog::FatalError(__FILE__, __LINE__, "Not Connect connectLogDB");
	...
	pServerInfoList->CServerInfoList::GetServerInfo(&LocalServerInfo, localSID);
	if( !CGlobalInfoDB::GetObj()->CGlobalInfoDB::IsColumnPrivateAdded() )
		CEnterTraceLog::GetObj()->CEnterTraceLog::FatalError(__FILE__, __LINE__, "This server requires PrivateIP and PrivatePort columns in ServerInfo table");
	if( LocalServerInfo.m_dwIP != CNetwork::GetObj()->CNetwork::GetLocalIP(true) )
		CEnterTraceLog::GetObj()->CEnterTraceLog::FatalError(__FILE__, __LINE__, "PUBLIC_IP misconfigured, check account.config.txt and ServerInfo table");
	if( LocalServerInfo.m_dwPrivateIP != CNetwork::GetObj()->CNetwork::GetLocalIP(false) )
		CEnterTraceLog::GetObj()->CEnterTraceLog::FatalError(__FILE__, __LINE__, "PRIVATE_IP misconfigured, check account.config.txt and ServerInfo table");
	...
	#define CONF(X) ( g_scrptConfig.CInfoScript::GetInfo(X) ) ? atoi(g_scrptConfig.CInfoScript::GetInfo(X))
	/////////////////////////////////////////////////////////////////////////////
	const int ASYNCPROCESS_THREADS = CONF("ASYNCPROCESS_THREADS") : 4;
	const int ASYNCPROCESS_POOL_COUNT = CONF("ASYNCPROCESS_POOL_COUNT") : 100000;
	const int COMPLETIONPORT_THREADS = CONF("COMPLETIONPORT_THREADS") : 3;
	...
	const int CHAR_PROCESS_OBJ_NUM = CONF("CHAR_PROCESS_OBJ_NUM") : 30;
