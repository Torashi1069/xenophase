#include "Character.h" // g_characterConfig
...
DWORD CNetwork::GetLocalIP(bool mode)
{
	...
	DWORD dwPublicIP = inet_addr(g_characterConfig.CInfoScript::GetInfo("PUBLIC_IP"));
	DWORD dwPrivateIP = inet_addr(g_characterConfig.CInfoScript::GetInfo("PRIVATE_IP"));
	if( mode && dwPublicIP != INADDR_NONE && dwPublicIP != INADDR_ANY ) return dwPublicIP;
	if( !mode && dwPrivateIP != INADDR_NONE && dwPrivateIP != INADDR_ANY ) return dwPrivateIP;
