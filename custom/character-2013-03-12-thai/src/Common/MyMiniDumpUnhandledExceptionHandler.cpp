...
/// singleton instance
CMyMiniDumpUnhandledExceptionHandler* CMyMiniDumpUnhandledExceptionHandler::m_cpSelf;
char CMyMiniDumpUnhandledExceptionHandler::m_MoudleFilePath[MAX_PATH];
char CMyMiniDumpUnhandledExceptionHandler::m_MoudleFileName[MAX_PATH];
...
bool CMyMiniDumpUnhandledExceptionHandler::CreateInstance()
{
	if( m_cpSelf != NULL )
		CEnterTraceLog::GetObj()->CEnterTraceLog::FatalError(__FILE__, __LINE__, "CMyMiniDumpUnhandledExceptionHandler::CreateInstance() m_cpSelf != NULL");

	m_cpSelf = new(std::nothrow) CMyMiniDumpUnhandledExceptionHandler();
	if( m_cpSelf == NULL )
		return false;
...
{
	if( m_cpSelf == NULL )
		CEnterTraceLog::GetObj()->CEnterTraceLog::FatalError(__FILE__, __LINE__, "CMyMiniDumpUnhandledExceptionHandler::DestroyInstance() m_cpSelf != NULL");
	...
	if( hDbgHelpDll != NULL )
	{
		...
		if( pDump != NULL )
		{
			...
			if( hDumpFile != INVALID_HANDLE_VALUE )
			{
				...
				if( reinterpret_cast<MINIDUMPWRITEDUMP>(pDump)(GetCurrentProcess(), GetCurrentProcessId(), hDumpFile, MiniDumpWithFullMemory, &ExInfo, NULL, NULL) )
				{// success.
					retval = TRUE;
				}
...
void CMyMiniDumpUnhandledExceptionHandler::MyStackWalk(const SYSTEMTIME& st, EXCEPTION_POINTERS* pExceptionInfo)
{
	...
	f.CMyFile::Write("*------------------------------------------------------------*\r\n");
	f.CMyFile::Write("날짜:%s\r\n", GetSystemTimeFmtStr(st)); //FIXME: "Date"
