...
template<typename T> T* CMemPool<T>::Allocate()
{
	...
	if( m_pFreeObjList != NULL )
	{
		if( m_pFreeObjList->m_bAllocated )
			CEnterTraceLog::GetObj()->NFileLog::WriteFmt(__FILE__, __LINE__, "already allocated - %s", __FUNCTION__);
...
template<typename T> bool CMemPool<T>::Free(T* pObject)
{
	...
	{// out of range
		CEnterTraceLog::GetObj()->NFileLog::WriteFmt(__FILE__, __LINE__, "out of range - %s", __FUNCTION__);
		m_csMemPool.Leave();
		return false;
	}
	...
	{// misaligned pointer
		CEnterTraceLog::GetObj()->NFileLog::WriteFmt(__FILE__, __LINE__, "invalid pos - %s", __FUNCTION__);
		m_csMemPool.Leave();
		return false;
	}
	...
	{// already freed
		CEnterTraceLog::GetObj()->NFileLog::WriteFmt(__FILE__, __LINE__, "already free - %s", __FUNCTION__);
		m_csMemPool.Leave();
		return false;
	}
