...
void CUserList::Insert(DWORD dwAID, __SUSER__& object)
{
	...
	if( m_pUser == NULL )
	{
		CEnterTraceLog::GetObj()->NFileLog::WriteFmt(__FILE__, __LINE__, "m_pUser is 0 func %s\n", __FUNCTION__);
		return;
	}
	...
	CSingleton<CCharacterDB>::getSingletonPtr()->CCharacterDB::SetOnline(true, object.m_dwAID, object.m_nSelectedGID); // [patch] update 'isonline' column
...
void CUserList::Delete(DWORD dwAID)
{
	...
	CSingleton<CCharacterDB>::getSingletonPtr()->CCharacterDB::SetOnline(false, dwAID); // [patch] update 'isonline' column
