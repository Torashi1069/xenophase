...
///
	...
	if( !DBProxy<CGlobalInfoDB,4>::instance(0)->CGlobalInfoDB::IsColumnPrivateAdded() )
	...
	if( gLocalServerInfo.m_dwIP != CNetwork::GetObj()->CNetwork::GetLocalIP(true) )
		CEnterTraceLog::GetObj()->CEnterTraceLog::FatalError(__FILE__, __LINE__, "PUBLIC_IP misconfigured, check character.config.txt and ServerInfo table");
	if( gLocalServerInfo.m_dwPrivateIP != CNetwork::GetObj()->CNetwork::GetLocalIP(false) )
		CEnterTraceLog::GetObj()->CEnterTraceLog::FatalError(__FILE__, __LINE__, "PRIVATE_IP misconfigured, check character.config.txt and ServerInfo table");
...
///
	...
	CSingleton<CCharacterDB>::getSingletonPtr()->CCharacterDB::SetOnline(false); // [patch] update 'isonline' column
...
///
	...
	#define CONF(X) ( g_characterConfig.CInfoScript::GetInfo(X) ) ? atoi(g_characterConfig.CInfoScript::GetInfo(X))
	/////////////////////////////////////////////////////////////////////////////
	const int ASYNCPROCESS_THREADS = CONF("ASYNCPROCESS_THREADS") : 5;
	...
	SYSTEM_INFO si;
	GetSystemInfo(&si);
	const int COMPLETIONPORT_THREADS = CONF("COMPLETIONPORT_THREADS") : min(8 * si.dwNumberOfProcessors, 64);

	g_compltport.CCompletionPort::Create(COMPLETIONPORT_THREADS);
	/////////////////////////////////////////////////////////////////////////////
	std::list<CProcess*> process_list;
	/////////////////////////////////////////////////////////////////////////////
	const int ZONE_NETWORK_BUFSIZE = CONF("ZONE_NETWORK_BUFSIZE") : 3000000;
	const int ZONE_PROCESS_OBJ_NUM = CONF("ZONE_PROCESS_OBJ_NUM") : 30;
	...
	const DWORD ZONE_IP = gLocalServerInfo.m_dwPrivateIP;
	const WORD ZONE_PORT = (WORD)gLocalServerInfo.m_nPrivatePort;
	...
	process_list.clear();
	/////////////////////////////////////////////////////////////////////////////
	const int ACC_NETWORK_BUFSIZE = CONF("ACC_NETWORK_BUFSIZE") : 30000000;
	const int ACC_PROCESS_OBJ_NUM = CONF("ACC_PROCESS_OBJ_NUM") : 1;
	...
	const char* szACC_IP = account_server_info.m_szPrivateIP;
	const WORD ACC_PORT = (WORD)account_server_info.m_nPrivatePort;
...
void MakeCHARACTER_INFO(const SCHARINFO* pCharInfo, CHARACTER_INFO* pCharacter_Info_target)
{
	...
	pCharacter_Info_target->virtue = pCharInfo->m_nVirtue;
	pCharacter_Info_target->honor = pCharInfo->m_nHonor;
	...
	pCharacter_Info_target->haircolor = static_cast<unsigned char>(pCharInfo->m_nHairColor);
