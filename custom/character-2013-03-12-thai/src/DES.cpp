...
// key must be 56 bits
// Anyway, the parity bits introduced in the des-how-to are discarded by the first permutation function
...
void DES::setKey(const unsigned char* k)
{
	...
	{
		unsigned char c=k[i];
		*(ptr++)=(c&0x80)>>7; if ((++hole) % 7 == 0) *(ptr++)=0;
		*(ptr++)=(c&0x40)>>6; if ((++hole) % 7 == 0) *(ptr++)=0;
		*(ptr++)=(c&0x20)>>5; if ((++hole) % 7 == 0) *(ptr++)=0;
		*(ptr++)=(c&0x10)>>4; if ((++hole) % 7 == 0) *(ptr++)=0;
		*(ptr++)=(c&0x08)>>3; if ((++hole) % 7 == 0) *(ptr++)=0;
		*(ptr++)=(c&0x04)>>2; if ((++hole) % 7 == 0) *(ptr++)=0;
		*(ptr++)=(c&0x02)>>1; if ((++hole) % 7 == 0) *(ptr++)=0;
		*(ptr++)=(c&0x01)>>0; if ((++hole) % 7 == 0) *(ptr++)=0;
	}
	...
	while (len>=8)
	{
		...
		for (int i=0; i<16; i++)
		{
			...
			for (int j=0; j<8; j++)
			{
				unsigned char c = S[j]
					[((*ERIndex)<<1)|(*(ERIndex+5))]
					[((*(ERIndex+1))<<3)|((*(ERIndex+2))<<2)|((*(ERIndex+3))<<1)|(*(ERIndex+4))<<0];
				*(ERIndexS++)=(c&0x08)>>3;
				*(ERIndexS++)=(c&0x04)>>2;
				*(ERIndexS++)=(c&0x02)>>1;
				*(ERIndexS++)=(c&0x01)>>0;
