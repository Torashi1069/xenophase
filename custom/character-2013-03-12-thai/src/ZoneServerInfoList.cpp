...
void CZoneServerInfoList::AddZoneServerInfo(SSERVERINFO* pServerInfo)
{
	...
	zoneserverinfo.m_MaxConcurrentPlayerNum = 0;
	zoneserverinfo.m_UpdateCnt = 0;
	zoneserverinfo.m_bOverPopulation = 0;
...
int CZoneServerInfoList::GetSID(DWORD in_dwIP, int in_nPort)
{
	int nSID = 0;
	m_csZoneServerInfoList.Enter();
	...
	for( size_t i = 0; i < m_vectorZoneServerInfoList.size(); ++i )
	{
		if( m_vectorZoneServerInfoList[i].m_dwIP == in_dwIP && m_vectorZoneServerInfoList[i].m_nPort == in_nPort
		||  m_vectorZoneServerInfoList[i].m_dwPrivateIP != 0 && m_vectorZoneServerInfoList[i].m_dwPrivateIP == in_dwIP && m_vectorZoneServerInfoList[i].m_nPrivatePort == in_nPort )
...
BOOL CZoneServerInfoList::IsZoneServer(DWORD in_dwIP, int in_nPort)
{
	BOOL result = FALSE;
	m_csZoneServerInfoList.Enter();
	...
	for( size_t i = 0; i < m_vectorZoneServerInfoList.size(); ++i )
	{
		if( m_vectorZoneServerInfoList[i].m_dwIP == in_dwIP )
		{
			m_vectorZoneServerInfoList[i].m_nPort = in_nPort;
			if( in_nPort != 0 )
			{
				result = TRUE;
				break;
		...
		if( m_vectorZoneServerInfoList[i].m_dwPrivateIP != 0 && m_vectorZoneServerInfoList[i].m_dwPrivateIP == in_dwIP && m_vectorZoneServerInfoList[i].m_nPrivatePort == in_nPort )
		{
			result = TRUE;
			break;
		}
...
bool CZoneServerInfoList::IsEveryZoneConnected() /// @custom // [patch] prevent login if there are any missing zone servers
{
	//TODO: when the code is made to update SZONESERVERINFO::m_bConnect, use that instead
	return( m_vectorZoneServerInfoList.size() == g_ZoneSvrStreamMgr.CStreamManager::GetConnectionCount() );
}
